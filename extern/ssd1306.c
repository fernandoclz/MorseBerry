 ...
 * * MODIFICACIONES:
 * Este archivo ha sido modificado por Fernando Liu en 2026
 * para adaptarlo a las necesidades del proyecto MorseBerry.
 * Cambios realizados: Comentadas funciones originales de ssd1306_WriteCommand y ssd1306_WriteData, incluidas librerias para i2c y otra version de las misma funciones.
 */
...
// Variable global para el File Descriptor del bus I2C
int i2c_fd = -1; 

// Enviar un comando al registro SSD1306
void ssd1306_WriteCommand(uint8_t byte) {
    if (i2c_fd < 0) return;
    uint8_t buffer[2] = {0x00, byte}; // 0x00 indica comando
...
void ssd1306_WriteData(uint8_t* buffer, size_t buff_size) {
    ...
    // El primer byte debe ser 0x40 para indicar que son datos
    uint8_t *data = malloc(buff_size + 1);
    data[0] = 0x40; 
    memcpy(&data[1], buffer, buff_size);
    
    write(i2c_fd, data, buff_size + 1);
    free(data);
}
/*
