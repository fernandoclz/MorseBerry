...
void* funcion_hilo_gpio(void* arg){
    ...
    while(1){
        ...
        else if (ret == 0 && pulsado == 0){ //si no hay eventos y el boton no se pulsa se revisaa si hay espacio largo
            int tiempo_ahora = obtener_tiempo_actual();
...
int main(int argc, char** argv){
    ...
    */
    ...
    if (!chip) { perror("Error al abrir chip"); return EXIT_FAILURE; }
    ...
    if (!request) { perror("Error solicitud linea"); goto close_chip; }
 ...
 /*
    ...
    */
    while (1) {
        ...
        if (lectura != 0) {
if (lectura == SIMBOLO_PUNTO) printf(".");
            else if (lectura == SIMBOLO_RAYA) printf("-");
            else if (lectura == SIMBOLO_ESPACIO) printf(" [FIN LETRA] ");
            else if (lectura == SIMBOLO_DESCONOCIDO) printf("?");
            fflush(stdout);
        }
